...
(function () {
  ...
  function normalizeMamaState(value) {
    ...
    const affection = Number(source.affection);
    return {
      affection: Number.isFinite(affection) ? Math.max(0, Math.min(255, Math.round(affection))) : defaults.affection,
      outfit: typeof source.outfit === 'string' && source.outfit.trim() ? source.outfit.trim() : defaults.outfit,
      expression: typeof source.expression === 'string' && source.expression.trim() ? source.expression.trim() : defaults.expression,
      mascotComment: typeof source.mascotComment === 'string' && source.mascotComment.trim() ? source.mascotComment.trim() : defaults.mascotComment,
      enaDialogue: typeof source.enaDialogue === 'string' && source.enaDialogue.trim() ? source.enaDialogue.trim() : defaults.enaDialogue
    };
  }
