...
export function renderVisualDashboard(root: HTMLElement, model: VisualDashboardViewModel): void {
  const isPortraitViewing = root.dataset.portraitViewing === 'true' || root.classList.contains('is-portrait-viewing');

  root.classList.add('visual-dashboard');
  root.classList.toggle('is-portrait-viewing', isPortraitViewing);
  root.dataset.appId = model.appId;
  root.dataset.portraitViewing = String(isPortraitViewing);
  root.replaceChildren(
    ...
    renderStatusShowcase(model.state, isPortraitViewing),
...
function renderStatusShowcase(state: MamaState, isPortraitViewing: boolean): HTMLElement {
  ...
  stage.append(figure, renderPortraitViewToggle(isPortraitViewing));
  frame.append(
    stage,
...
function renderPortraitViewToggle(isPortraitViewing: boolean): HTMLElement {
  const button = createElement('button', {
    className: `portrait-view-toggle${isPortraitViewing ? ' is-active' : ''}`,
    attributes: {
      type: 'button',
      title: '立绘鉴赏',
      'aria-label': '立绘鉴赏',
      'aria-pressed': String(isPortraitViewing)
    }
  });
  ...
  button.addEventListener('click', (event) => {
    event.stopPropagation();
    const dashboard = button.closest<HTMLElement>('.visual-dashboard');
    const nextState = !dashboard?.classList.contains('is-portrait-viewing');

    dashboard?.classList.toggle('is-portrait-viewing', nextState);
    if (dashboard) dashboard.dataset.portraitViewing = String(nextState);
